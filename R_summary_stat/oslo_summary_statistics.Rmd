---
title: "Oslo sammendragsstatistikk"
...
#    output_dir = "../../reports", output_format = "all")})
params:
  region_str:
    label: "Kommune"
    value: Oslo
...
calc_summary <- function(data, x, classify_other = FALSE) {
  ...
  print(abs(sum(summary$Probability) - 1))
  ...
  if (classify_other) {
    data_summary <- summary %>%
      dplyr::filter(Perc >= 2) %>%
      dplyr::bind_rows(dplyr::summarise(dplyr::filter(summary, Perc < 2), 
...
lookup_attr <- readxl::read_excel(
    ...
    sheet = catalog$lookup_attr$sheet_2,
    col_names = TRUE,
    col_types = NULL,
    na = "",
    skip = 0
...
df <- config$convert_csv_to_parquet(
    csv_file = catalog_municipality$bytraer$filepath,
...
# 1. rename columns using lookup_attr
# -----------------------------------

# if value in lookup_attr$name_python is empty delete row

lookup_attr <- lookup_attr %>%
    dplyr::filter(!is.na(name_python))

# remove "CROWN_ID_2021" and "GEO_RELATION"
lookup_attr <- lookup_attr %>%
    dplyr::filter(name != "CROWN_ID_2021" & name != "GEO_RELATION")

# if df colname is not in lookup_attr$name delete colname
if (any(!(colnames(df) %in% lookup_attr$name))) {
    df <- df %>%
        dplyr::select(lookup_attr$name)
}

# trim lookup_attr$name and lookup_attr$name_python
lookup_attr$name <- trimws(lookup_attr$name)
lookup_attr$name_python <- trimws(lookup_attr$name_python)

# rename all df colnames to name_python
df <- df %>%
    dplyr::rename_all(~lookup_attr$name_python[match(., lookup_attr$name)])
...
# 4. Modify ecosystem-columns
# ---------------------------
# if < 0 set to NA
# "co2_storage_nkr" not available in oslo 
...
# set to float and round to 2 decimals
...
# ---------------
# itree_spec == 1 and  == 1 
df_school_itree <- df[df$itree_spec == 1 & df$schools == 1,]
df_school <- df[df$schools == 1,]
n_school_itree <- nrow(df_school_itree)
df_school$class <- "Skoletraer"
# 1. i-Tree Eco trees
# -------------------
...
# MERGE DATA
# ----------
data_merged <- rbind(df, df_school, df_itree, df_z1, df_z2)
...
| Skoletrær              | Totalt antall trær plassert i undervisningsområde      | `r nrow(df_school)` (i-Tree = `r nrow(df_school_itree)`) |
...
Treslag som har en sannsynlighet på mindre enn 2% er klassifisert som "Andre treslag".
...
# Groups: "Alle Traer", "i-Tree", "Skoletræer", Sone 1",  "Sone 2", "Sone 3"
summary_list <- lapply(
    X = df_list, 
...
plot_school <- nina_plot$probability_plot(
    data = summary_list$df_school,
    Var1 = "norwegian_name",
    title = "Skoletrær",
    ymax = params_municipality$norwegian_name$ymax
    ) +
...
```{r "TREE ATTRIBUTES", echo=FALSE, results='hide',message=FALSE, warning=FALSE, fig.height=6, fig.width = 14}
...
```{r "MEAN_PER _CLASS", echo=FALSE, results='hide',message=FALSE, warning=FALSE}
# CALC MEAN PER CLASS
# -------------------
...
es_cols <- parameters$es_columns_oslo
df_mean_per_class <- df_itree[, es_cols]

```

```{r "REGULATING_ECOSYSTEM_SERVICES", echo=FALSE, results='hide',message=FALSE, warning=FALSE, fig.height=3, fig.width = 14}
# 1. Prepare plot parameters
# --------------------------
...
```{r "KARBON_LAGRING", echo=FALSE, results='hide',message=FALSE, warning=FALSE, fig.height=12, fig.width = 7}
...
plot_co2_storage <- plot(
    plot_species, 
    plot_other[2:4], 
    "Gjennomsnittlig karbonlagring (kg) \nper treslag, stammediameter, kroneareal, høyde")
print(plot_co2_storage)
```

<font size="0.8" face="Arial">\*Treslag med gjennomsnittlig karbonlagring \< `r params_bar$mean_threshold` kg vises ikke i plottet. </font>

```{r "KARBON_BINDING", echo=FALSE, results='hide',message=FALSE, warning=FALSE, fig.height=12, fig.width = 7}
...
plot_co2_binding <- plot(
    plot_species, 
    plot_other[2:4], 
    "Gjennomsnittlig årlig karbonbinding (kg/år) \nper treslag, stammediameter, kroneareal, høyde")
print(plot_co2_binding)
```

<font size="0.8" face="Arial">\*Treslag med årlig karbonbinding \< `r params_bar$mean_threshold` kg/år vises ikke i plottet. </font>

```{r OVERFLATEAVRENNING, echo=FALSE, results='hide',message=FALSE, warning=FALSE, fig.height=12, fig.width = 7}
...
<font size="0.8" face="Arial">\*Treslag med gjennomsnittlig reduksjon av overflateavrenning \< `r params_bar$mean_threshold` m3/år vises ikke i plottet. </font>

```{r POLLUTION, echo=FALSE, results='hide',message=FALSE, warning=FALSE, fig.height=12, fig.width = 7}
...
<font size="0.8" face="Arial">\*Treslag med gjennomsnittlig reduksjon av luftforurensing \< `r params_bar$mean_threshold` g/år vises ikke i plottet. </font>
